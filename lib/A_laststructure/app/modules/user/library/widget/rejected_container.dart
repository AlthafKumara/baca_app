import 'package:baca_app/A_laststructure/app/core/color/app_color.dart';
import 'package:baca_app/A_laststructure/app/core/font/app_text_style.dart';
import 'package:baca_app/A_laststructure/app/data/model/book_model.dart';
import 'package:baca_app/A_laststructure/app/data/model/borrow_model.dart';
import 'package:baca_app/A_laststructure/app/modules/user/library/controllers/borrow_controller.dart';

import 'package:baca_app/A_laststructure/app/modules/user/library/widget/card_status_container.dart';
import 'package:baca_app/A_laststructure/app/modules/user/library/widget/empty_container.dart';
import 'package:flutter/widgets.dart';
import 'package:get/get.dart';

class RejectedContainer extends StatelessWidget {
  final borrowC = Get.find<BorrowController>();
  RejectedContainer({super.key});

  @override
  Widget build(BuildContext context) {
    return borrowC.obx(
      (state) {
        final list = state?.where((e) => e.status == Status.rejected).toList();

        if (list!.isEmpty) {
          return EmptyContainer();
        }
        return ListView.builder(
          itemCount: list.length,
          itemBuilder: (context, index) {
            final book = list[index].book as Book;
            final borrow = list[index];
            return CardStatusContainer(status: Status.rejected, borrow: borrow);
          },
        );
      },
      onEmpty: EmptyContainer(),
      onError: (error) {
        print(error);
        return Center(
          child: Text(
            error.toString(),
            style: AppTextStyle.body1(
              color: AppColor.Neutral900,
              fontWeight: AppTextStyle.regular,
            ),
          ),
        );
      },
    );
  }
}
